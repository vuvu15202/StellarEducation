...
const QUESTION_BANK_ID =
  new URL(window.location.href).searchParams.get("questionbankid") || 0;
...
const ApiService = {
  ...
  getExamination: function (successCallback) {
    sendRequest(
      "GET",
      `/QuestionBanks/editquestion/${QUESTION_BANK_ID}/reading`,
      {},
      successCallback
    );
  },
  ...
  // Cập nhật dữ liệu
  downloadTemlateExcel: function (successCallback) {
    sendRequest(
      "GET",
      `/QuestionBanks/template/download/reading`,
      {},
      successCallback
    );
  },
...
$(document).ready(function () {
  ...
  ApiService.getExamination(function (data) {
    ...
    document.getElementById("countdown").textContent = formatTime(
      data.readingJSON.time
    );
  ...
  $(document).on("click", "button.save-picture", function () {
    ...
    formData.append("forQuestion", "reading");
  ...
  $(document).on("click", "div#downloadexcel", function () {
    var token = getCookie("token");
    $.ajax({
      url: `${decodeURIComponent(API_BASE_URL)}/QuestionBanks/template/download/reading`,
      type: "get",
      headers: {
        Authorization: "Bearer " + token,
      },
      ...
      success: function (blob, status, xhr) {
        ...
        a.href = downloadUrl;
        a.download = "templateReading.xlsx"; // Tên file bạn muốn tải về
