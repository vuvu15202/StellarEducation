...
const ApiService = {
  ...
  getExamination: function (successCallback) {
    sendRequest(
      "GET",
      `/ExamCandidates/review/${EXAM_CANDIDATE_ID}/reading`,
      {},
      successCallback
    );
  },
      ...
      var answers = "";
        ...
        title +=
          ...
          question.title.replace(
            /\(\.\.\.\)|\(\u2026\)/g,
              `<span data-questionno="${question.questionNo}" name="answer-part${part.partNo}-group${group.groupNo}-question${question.questionNo}" class="dropbox fw-bold ${isCorrect ? 'bg-success-custom' : 'bg-danger-custom'}">${submitedAnswerContent}</span>`
          ) +
