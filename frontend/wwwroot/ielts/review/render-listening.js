...
const ApiService = {
  ...
  getExamination: function (successCallback) {
    sendRequest(
      "GET",
      `/ExamCandidates/review/${EXAM_CANDIDATE_ID}/listening`,
      {},
      successCallback
    );
  },
...
$(document).ready(function () {
  ...
  function updateCountdown() {
    ...
    if (countdownTime >= 0) {
      countdownElement.textContent = formatTimeCountdown(countdownTime); // Cập nhật giao diện
      ...
      if (countdownTime <= 60) {
        ...
        countdownElement.classList.remove("zoom-text");
      }
...
function renderGroups(part, submitedAnswers) {
  ...
  groups.forEach(function (group) {
    ...
    if (group.fileType == "jpg" || group.fileType == "png" || group.fileType == "jpeg")
      fileurl = `<img src="${group.fileURL}" width="90%;" />`;
    ...
    scrollspy.append(`
            ...
            <div id="group-part${part.partNo}-group${group.groupNo}" class="col-6">
    ...
    if (group.type == "dropbox") {
      ...
      var answers = "";
        ...
        scrollspyitem.append(`
                ...
                `);
        var submitedAnswer = submitedAnswers.find(s => s.QuestionNo == question.questionNo);
        var isCorrect = submitedAnswer != undefined && submitedAnswer.SubmitedAnswer == question.correctAnswer;
        var submitedAnswerContent = submitedAnswer != undefined ? `${submitedAnswer.SubmitedAnswer}<button data-questionno="10" class="clear-btn" style="display: inline-block;">x</button>` : `${question.questionNo}`;
        title +=
          `<span id="list-item-${question.questionNo}" class="fw-bold me-3">${question.questionNo}</span>` +
          question.title.replace(
            /\(\.\.\.\)|\(\u2026\)/g,
              `<span data-questionno="${question.questionNo}" name="answer-part${part.partNo}-group${group.groupNo}-question${question.questionNo}" class="fw-bold ${isCorrect ? 'bg-success-custom' : 'bg-danger-custom'} dropbox ">${submitedAnswerContent}</span>`
        ) +
          `<div class="alert alert-success alert-dismissible fade show m-3" role="alert">
                <strong>Giải thích! </strong> ${question.explainString !== null ? question.explainString : ""}.
        ...
        scrollspyitem.append(`
          ...
          if (group.type == "radio") {
              ...
              var answer = $.map(question.answers, function (answer) {
                  var bg_success = submitedAnswer != undefined &&
                      submitedAnswer.SubmitedAnswer == answer &&
                      submitedAnswer.SubmitedAnswer == question.correctAnswer;
                  var bg_danger = submitedAnswer != undefined &&
                      submitedAnswer.SubmitedAnswer == answer &&
                      submitedAnswer.SubmitedAnswer != question.correctAnswer;
                  var isChecked = submitedAnswer != undefined && submitedAnswer.SubmitedAnswer == answer;
                  return `<div class="col-12 pb-2  
                            ${bg_success ? 'bg-success-custom' : ''}
                            ${bg_danger ? 'bg-danger-custom' : ''}">
                            <input type="radio" ${isChecked ? 'checked' : ''} name="answer-part${part.partNo}-group${group.groupNo}-question${question.questionNo}"  data-questionno="${question.questionNo}" value="${answer}" class="card-text answer text-success" /> <label class="ms-1">${answer}</label>
                        </div>
                        `;
          });
          return `
            ...
            `.replace(`(answer)`, answer.join(""));
              ...
              var answer = $.map(question.answers, function (answer) {
                  var bg_success = submitedAnswer != undefined &&
                      submitedAnswer.SubmitedAnswer.split('-').includes(answer) &&
                      question.correctAnswer.split('-').includes(answer);
                  var bg_danger = submitedAnswer != undefined &&
                      submitedAnswer.SubmitedAnswer.split('-').includes(answer) &&
                      !question.correctAnswer.split('-').includes(answer);
                  var isChecked = submitedAnswer != undefined && submitedAnswer.SubmitedAnswer.includes(answer);
                  return `<div class="col-12 pb-2 ${bg_success ? 'bg-success-custom' : ''} ${bg_danger ? 'bg-danger-custom' : ''}">
                    <input type="checkbox" ${isChecked ? 'checked' : ''} name="answer-part${part.partNo}-group${group.groupNo}-question${question.questionNo}" data-questionno="${question.questionNo}" value="${answer}" class="card-text answer text-success" /> <label class="ms-1">${answer}</label>
                </div>
                `;
              });
              return `
          ...
          var title = question.title.replace(
            /\(\.\.\.\)|\(\u2026\)/g,
              `<input data-questionno="${question.questionNo}" value='${submitedAnswer != undefined ? submitedAnswer.SubmitedAnswer : ''}' type="text" name="answer-part${part.partNo}-group${group.groupNo}-question${question.questionNo}" class="${isCorrect ? 'bg-success-custom' : 'bg-danger-custom'}">`
          );
          return `
                <div class="card">
                    <div id="list-item-${question.questionNo}" class="card-body">
