...
$(document).ready(function () {
    ...
    function ShowReport() {
        $.ajax({
            url: "http://localhost:5000/api/StudentFees/report",
            type: "get",
            headers: {
    ...
    function ShowProjectBills() {
        ...
        $.ajax({
            url: `${decodeURIComponent(API_BASE_URL)}/StudentFees/CoursesStudentFees`,
            type: "get",
            headers: {
                "Authorization": "Bearer " + token,
            },
            ...
            success: function (result, status, xhr) {
                ...
                $.each(result, function (index, value) {
                    $("#projectbills").append(`
                        ...
                        <span class="text-xs">typeText</span>
                        </div>
    ...
    function showHandlingbills(){
      $("#handlingbills").html("");
        $.ajax({
            url: `${decodeURIComponent(API_BASE_URL)}/StudentFees/handlingbills`,
            type: "get",
            headers: {
                "Authorization": "Bearer " + token,
            },
                ...
                $.each(result, function (index, value) {
                  ...
                  $("#handlingbills").append(`
                      ...
                      <div class="d-flex flex-column">
                        ...
                        <span class="mb-2 text-xs">Project Title: <span class="text-dark font-weight-bold ms-sm-2"><a href="javascript:void(0)">${value.course.name}</a></span></span>
                      ...
                      <div class="ms-auto text-end">
                        ...
                        <a class="btn btn-link text-dark px-3 mb-0 handling-bill-details" data-id="${value.studentFeeId}" href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#staticBackdropp">
    ...
    $(document).on('click', '#btnExport', function () {
        ...
        $.ajax({
            url: "http://localhost:5000/api/StudentFees/export",
            type: "post",
            headers: {
                "Authorization": "Bearer " + token,
            },
    ...
    function showListOfBills(){
        ...
        $.ajax({
            url: "http://localhost:5000/api/StudentFees/GetStudentFeesByDate",
            type: "post",
            headers: {
                "Authorization": "Bearer " + token,
            },
                    ...
                    if(value.errorCode === "0" || value.errorCode === "00"){
                        ...
                        <li class="list-group-item border-0 d-flex justify-content-between ps-0 mb-2 border-radius-lg">
                            ...
                            <div class="d-flex flex-column">
                                ...
                                <span class="text-xs">${value.dateOfPaid}</span>
                            </div>
                            </div>
    ...
    $(document).on('click', '.viewprojectbills', function(){
        ...
        $.ajax({
            url: "http://localhost:5000/api/StudentFees/CourseStudentFees/" + id,
            type: "get",
            headers: {
                "Authorization": "Bearer " + token,
            },
            contentType: "application/json",
            success: function (result, status, xhr) {
                $.each(result.studentFees, function (index, value) {
                    console.log(value)
                    $("#project-bill-detail").html('');
    ...
    $(document).on('click', '.handling-bill-details', function(){
        let id = $(this).attr('data-id');
        var bill = handlingbills.find(obj => obj.studentFeeId == id);
        ...
        $("#handling-bill-details").append(`
            <table class="table">
                ...
                <tbody>
                <tr> <td>OrderId</td> <td>${bill.studentFeeId}</td> </tr>
                <tr> <td>ProjectId</td> <td>${bill.course.name}</td> </tr>
                ...
                <tr> <td>DateOfDonation</td> <td>${bill.dateOfPaid}</td> </tr>
                </tbody>
            </table>
          `); 
    });
    ...
    $('#inputSearchProject').on('input', function () {
        ...
        var filteredStudents = projectbills.filter(function (course) {
