...
const ApiService = {
	getCourses: function (successCallback) {
		sendRequest("GET", `/Courses`, {}, successCallback);
	},
...
$(document).ready(function () {
	...
	ApiService.getCourses(function (data) {
		courses = data;
	...
	function renderCourses(page) {
		...
		coursesToDisplay.forEach(course => {
			...
			courseNumberOfEnrolled.classList.add('course_info');
			courseNumberOfEnrolled.innerHTML = `<i class="fa fa-graduation-cap mr-2" aria-hidden="true"></i><span>${course.numberEnrolled} đang học</span>`;
			...
			coursePrice.innerHTML = course.price != 0 ? `<a class="text-danger text-bold" href="/Home/contact">Liên hệ</a>` : 'Miễn phí';
